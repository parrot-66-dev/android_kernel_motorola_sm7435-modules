...
#ifndef PT_TOUCH_LOG_DISABLE
		/* Touch Debug Log */
		if (tch->abs[PT_TCH_E] == PT_EV_LIFTOFF) {
			pt_debug(dev, DL_WARN, "TP[R] t=%d x=%d y=%d z=%d M=%d m=%d o=%d obj=%d tip=%d\n",
					t, tch->abs[PT_TCH_X], tch->abs[PT_TCH_Y], tch->abs[PT_TCH_P],
					tch->abs[PT_TCH_MAJ], tch->abs[PT_TCH_MIN], tch->abs[PT_TCH_OR],
					tch->abs[PT_TCH_O], tch->abs[PT_TCH_TIP]);
		} else if (tch->abs[PT_TCH_E] == PT_EV_TOUCHDOWN) {
			pt_debug(dev, DL_WARN, "TP[P] t=%d x=%d y=%d z=%d M=%d m=%d o=%d obj=%d tip=%d\n",
					t, tch->abs[PT_TCH_X], tch->abs[PT_TCH_Y], tch->abs[PT_TCH_P],
					tch->abs[PT_TCH_MAJ], tch->abs[PT_TCH_MIN], tch->abs[PT_TCH_OR],
					tch->abs[PT_TCH_O], tch->abs[PT_TCH_TIP]);
		}
...
static int pt_xy_worker(struct pt_mt_data *md)
{
	...
	if (tch.hdr[PT_TCH_LO]) {
		...
		if (md->pdata->flags & PT_MT_FLAG_NO_TOUCH_ON_LO) {
			...
			if (md->detect_large == false) {
				pt_debug(dev, DL_WARN, "%s: detect Large object(status:0x%d) Noise Level = %d\n",
					__func__, tch.hdr[PT_TCH_LO], tch.hdr[PT_TCH_NOISE]);
			}
			//Set Large Detect Flag
			md->detect_large = true;
			num_cur_tch = 0;
		}
	} else {
		// Skip detection until all finger release
		if (num_cur_tch == 0) {
			if (md->detect_large == true) {
				pt_debug(dev, DL_WARN, "%s: Large object release\n", __func__);
			}
...
static int pt_startup_attention(struct device *dev)
{
	...
	/* Touch Debug Log */
	if (md->num_prv_rec != 0) {
		pt_debug(dev, DL_WARN, "TP[AR] Touch All Release\n");
	}
#endif
	pt_mt_lift_all(md);
	md->detect_large = false;
